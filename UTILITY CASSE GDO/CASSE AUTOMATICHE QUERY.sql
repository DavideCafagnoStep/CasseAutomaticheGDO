...
SELECT s.sell_pro_id AS pro_id, (SELECT p.pro_description  FROM products p WHERE p.pro_id = s.sell_pro_id) AS product, sum(s.sell_price) as total FROM sells s WHERE s.sell_date = '2024-02-23' GROUP BY s.sell_pro_id;
