...
public class QueryConstants {
	...
	public static final String INCASSO_DATA_UNA_GIORNATA_PER_ARTICOLO = "SELECT s.sell_pro_id AS pro_id, (SELECT p.pro_description  FROM products p WHERE p.pro_id = s.sell_pro_id) AS product, sum(s.sell_quantity) as quantity, sum(s.sell_price) as total FROM sells s WHERE s.sell_date = ? GROUP BY s.sell_pro_id;";
